...
public class UserController extends HttpServlet {
	 ...
	 *             Method to build the pagination logic
	 */
	private void buildPagination(HttpServletRequest request)
			throws DatabaseOperationException, ApplicationException {
		int offset = 0;
		int ofsetLmt = 0;
		if (request.getParameter("offset") != null) {
			offset = Integer
					.parseInt(request.getParameter("offset").toString());
		}
		int per_page = 5;
		...
		if (offset > 1) {
			ofsetLmt = offset * per_page - per_page;
			max_page = offset * per_page;
		} else
			max_page = per_page;

		List<EmployeeTO> emplist = employeeBO.getAllEmployeesByPagination(
				String.valueOf(max_page), String.valueOf(ofsetLmt));
		request.setAttribute("employees", emplist);
		int userCnt = employeeBO.getAllEmployeeCount();
